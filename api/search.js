export default async function handler(req, res) {
    ...
    for (const q of queries.slice(0,3)) {
      try {
        ...
        for (const ch of (dd.items||[])) {
          ...
          if(bio.includes('learn')||bio.includes('tutorial')||bio.includes('education')||bio.includes('cours'))types.push('education');
          ...
          results.push({
            ...
            reason:subsF+' subs · '+eng+'% eng'+(hasDeal?' · Deal '+exchName+' (puede cambiar)':' · Sin acuerdo de exchange'),
  ...
  // ── APIFY TWITTER — usando Cheerio Scraper (gratuito en plan free) ────────
  if (APIFY_TOKEN && APIFY_TOKEN!=='PENDING' && (!platforms.length||platforms.includes('Twitter'))) {
    for (const q of queries.slice(0,2)) {
      try {
        // Usamos nitter (instancia pública) para buscar usuarios sin cookies
        const nitterUrl = 'https://nitter.net/search?q='+encodeURIComponent(q)+'&f=users';
        const apifyRes = await fetch(
          'https://api.apify.com/v2/acts/apify~cheerio-scraper/run-sync-get-dataset-items?token='+APIFY_TOKEN+'&timeout=40&memory=256',
          {
            method: 'POST',
            headers: {'Content-Type':'application/json'},
            body: JSON.stringify({
              startUrls: [{url: nitterUrl}],
              pageFunction: `async function pageFunction(context) {
                const { $ } = context;
                const users = [];
                $('.timeline-item,.user-card').each((i, el) => {
                  const handle = $(el).find('.username,.user-name').first().text().trim().replace('@','');
                  const name = $(el).find('.fullname,.display-name').first().text().trim();
                  const bio = $(el).find('.tweet-content,.bio,.user-bio').first().text().trim();
                  const followersText = $(el).find('.followers,.following').first().text().trim();
                  if (handle && handle.length > 1) {
                    users.push({ handle: '@'+handle, name: name||handle, bio, followersText, url: 'https://x.com/'+handle });
                  }
                });
        ...
        if (!apifyRes.ok) continue;
        const users = await apifyRes.json();
        for (const u of (Array.isArray(users) ? users : [])) {
          ...
          seen.add(u.handle);
          results.push({
            name:u.name||u.handle, handle:u.handle,
            platform:'Twitter', url:u.url||('https://x.com/'+u.handle.replace('@','')),
            market:markets[0]||'Global', flag:getFlag(''),
            followers:'?K', folNum:10000, engagement:'6.0%',
            niche:types[0], contentType:types,
            hasExistingDeal:hasDeal, currentExchange:hasDeal?(exchKws.find(e=>bio.includes(e))||null):null,
            reason:'Encontrado en Twitter buscando: '+q+(hasDeal?' · Tiene deal de exchange':' · Sin acuerdo previo'),
  ...
  return res.status(200).json({
    ...
    queries, source:'yt_apify_cheerio_v5'
  });
}
